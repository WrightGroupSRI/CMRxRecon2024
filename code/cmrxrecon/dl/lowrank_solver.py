...
from cmrxrecon.utils import ifft_2d_img, fft_2d_img, view_as_real, view_as_complex

from cmrxrecon.dl.lowrank_varnet import model_step
...
class model_step(nn.Module):
    ...
    def pass_spatial_basis_through_model(self, spatial_basis):
        ...
        # norm spatial basis
        normed_basis, max = self.norm(spatial_basis)
        ...
        denoised_spatail_basis = self.unnorm(denoised_spatail_basis, max)
    ...
    def pass_temporal_basis_through_model(self, temporal_basis):
        ...
        temporal_basis = temporal_basis.reshape(b, cmplx*sv, t)
        #
        ...
        print(denoised_temporal_basis.shape)
    ...
    def norm(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        # instance norm
        max = x.abs().amax(dim=(-1, -2), keepdim=True)
        return x/max, max

    def unnorm(self, x: torch.Tensor, max: torch.Tensor) -> torch.Tensor:
        x = x * max
        return x
